...
static void chip8_screen_check_bounds(int x, int y){
    assert(x >= 0 && x < CHIP8_WINDOW_WIDTH);
    assert(y >= 0 && y < CHIP8_WINDOW_HEIGHT);
}
...
bool chip8_screen_draw_sprite(struct chip8_screen* screen, int x, int y, const char* sprite, int num)
{
    ...
    for (int ly = 0; ly < num; ly++)
    {
        ...
        for (int lx = 0; lx < 8; lx++)
        {
            ...
            if (screen->pixels[(ly+y) % CHIP8_WINDOW_HEIGHT][(lx+x) % CHIP8_WINDOW_WIDTH])
            {
                pixel_collison = true;
            }

            screen->pixels[(ly+y) % CHIP8_WINDOW_HEIGHT][(lx+x) % CHIP8_WINDOW_WIDTH] ^= true;
        }
    }
